...
INTERFACE
  ...
  SUBROUTINE build_rectilinear_grid(rank, size, extent, &
    x_coords, y_coords, ghost_levels, grid_name) bind(c)
    ...
    REAL(c_double), DIMENSION(*), INTENT(IN) :: x_coords, y_coords
    CHARACTER(kind=c_char), DIMENSION(*), INTENT(IN) :: grid_name
  ...
  SUBROUTINE set_particle_position(id, x, y, grid_name) bind(c)
...
CONTAINS
  ...
  SUBROUTINE init_coproc(step, time)  
   ...
   ! ENDDO
   ALLOCATE(dens(1-ng:nx+ng, 1-ng:ny+ng, n_species))
  ...
  SUBROUTINE run_coproc(step, time)
    ...
    extent = (/ nx_global_min - ng, nx_global_max + ng, ny_global_min - ng, ny_global_max + ng, 0, 0 /)
    zero_extent = (/ nx_global_min, nx_global_max, ny_global_min, ny_global_max, 0, 0 /)
    whole_extent = (/ 1 - ng, nx_global + ng, 1 - ng, ny_global + ng, 0, 0 /)
    origin = (/ x_grid_min, y_grid_min, 0.0_num /)
    spacings = (/ dx, dy, 0.0_num /)
    ...
    IF (flag /= 0) THEN
     ...
     ! CALL build_rectilinear_grid(rank, nproc, extent, x, y, ng, "EM Field"//C_NULL_CHAR)
     ...
     ! CALL set_vector_field(rank, ex, ey, ez, "E (V/m)"//C_NULL_CHAR, "EM Field"//C_NULL_CHAR)
     ! CALL set_vector_field(rank, bx, by, bz, "B (T)"//C_NULL_CHAR, "EM Field"//C_NULL_CHAR)
     ! CALL set_vector_field(rank, jx, jy, jz, "J (A/m^2)"//C_NULL_CHAR, "EM Field"//C_NULL_CHAR)
      ...
      DO ispecies = 1, n_species
        ...
        CALL do_field_mpi_with_lengths(dens(:, :, ispecies), ng, nx, ny)
        CALL set_scalar_field(rank, dens(:, :, ispecies), &
            "Number Density - "//TRIM(species_list(ispecies)%name)//" (m^-3)"//C_NULL_CHAR, "Grid"//C_NULL_CHAR)
      ENDDO

     !   !CALL set_scalar_field(rank, species_list(ispecies)%initial_conditions%density, &
     !   !  "n (m^-3)"//C_NULL_CHAR, "EM Field"//C_NULL_CHAR)
     ...
     !       "P (kg*m/s)"//C_NULL_CHAR, "Particles - "//TRIM(species_list(ispecies)%name)//C_NULL_CHAR)
